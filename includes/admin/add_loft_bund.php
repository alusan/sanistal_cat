...
if ($lb_sani!=NULL && $lb_type!=NULL && isset($_FILES['lb_image'])) {
	//STAGE 1 - add	to database and retrieve the id
	mysql_query("INSERT INTO loft_bund (sani_nr, bund_loft, billednavn) VALUES('$lb_sani', '$lb_type', 'filler');") 
		or die(mysql_error());
		
	$last_result = mysql_query("SELECT * FROM loft_bund ORDER BY lb_id DESC LIMIT 1", $conn); 
	$last = mysql_fetch_array($last_result); 
	$loftbund_id = $last['lb_id'];
	...
	mysql_query("UPDATE loft_bund SET billednavn = '$filename' WHERE lb_id = '$loftbund_id';") 
			or die(mysql_error());
